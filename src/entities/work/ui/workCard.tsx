    ...
    return (
        <li className='mediaWork'>
            <div className="mediaWork_container">
                {typeWork === "VIDEO" && media?.[0]?.mediaLink && <Video src = {media[0].mediaLink}/>}
                {typeWork === "IMAGE" && media?.[0]?.mediaLink && 
                    ...
                    />}
                <MediaOverlay prize={prize} user={user} imageCards={typeWork === 'IMAGE'}/>
                <button type='button' className='mediaWork_container_openModal' onClick={onOpenModal}>dada</button>
            </div>
